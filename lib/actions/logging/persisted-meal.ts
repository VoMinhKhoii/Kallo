...
export function extractNutritionValues(
  ...
  // Keyed by `keyof NutritionValues`, so the accumulator IS the return type
  // once the loop has written all 28 nutrients — no cast back out.
  const result = {} as Record<keyof NutritionValues, number | null>;
  for (const key of NUTRITION_KEYS) {
    const val = row[key];
