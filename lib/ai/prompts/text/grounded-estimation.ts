...
<macro_rule>
  ...
  ACCEPTED candidate, prep_notes EMPTY — the server replaces protein and carb with the DB row: keep those two brief (flat triples at base are fine); your effort belongs in grossG, refusePct, and fat.
  ...
  ACCEPTED candidate, prep_notes NON-EMPTY — the server unlocks protein and carb: reflect the user's modifier in them. Move only what the note physically implies.
  ...
  NO accepted candidate (match_status="unmatched", or selectedCandidateId="none"): emit ABSOLUTE LOW/MID/HIGH proteinG, carbohydrateG and fatG for the as-eaten portion from cuisine knowledge — they are the truth; nothing overrides them. ${DENSITY_PRIORS[locale]} Keep 4P + 4C + 9F under 900 kcal/100g.
</macro_rule>
...
${outputIngredient}
  proteinG, carbohydrateG and fatG are REQUIRED on every ingredient, matched or not — never calories. 0 is a valid value; an omitted field is a schema violation and the whole response is rejected.
