...
<vessel_rule>
  When a <meal_item> carries serve_total_guard_g="L-H": the SERVED total of that dish — every ingredient at full serving with broth/liquid at 100% — must land inside [L, H] grams. resolved_grams anchors and explicit user weights are AUTHORITATIVE: never rescale them; fit the band by adjusting only non-anchored ingredients. EATEN vs SERVED: emit grams for what was EATEN — solids fully eaten; broth/liquid grams = served broth × broth_consumption from <user_context> (leave_it ≈ 10%, some ≈ 50%, finish_it = 100%). The eaten total may fall below the band when broth is not drunk; the SERVED total must still be plausible for the vessel. Portion cues on ingredients shift within the band: ít/little ≈ −30%, nhiều/extra ≈ +40%, nửa/half = ×0.5.
</vessel_rule>`;
