...
vi.mock('@/lib/ai/pipeline/telemetry/run-telemetry', async () => {
  const actual = await vi.importActual<
    typeof import('@/lib/ai/pipeline/telemetry/run-telemetry')
  >('@/lib/ai/pipeline/telemetry/run-telemetry');
  return {
    ...actual,
...
const { analyzeMealV2 } = await import(
  '@/lib/ai/pipeline/grounded/orchestrator'
);
