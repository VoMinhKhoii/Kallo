...
function sampleFromFields(fields: Record<string, unknown>): unknown {
  return Object.fromEntries(
    Object.entries(fields).map(([key, field]) => {
      if (is(field, Column)) return [key, sampleValue(field)];
      if (field && typeof field === 'object' && !('getSQL' in field)) {
        return [key, sampleFromFields(field as Record<string, unknown>)];
      }
...
export function sampleRowFor(query: CapturedQuery, table: Table): unknown {
  if (query.fields) return sampleFromFields(query.fields);
  return sampleFromFields(getTableColumns(table));
}
...
export function createExportDb(
  ...
  const select = vi.fn((fields?: Record<string, unknown>) => {
    ...
    const self = Promise.resolve().then(
      () => rows(query) ?? [sampleRowFor(query, fromTable as Table)]
