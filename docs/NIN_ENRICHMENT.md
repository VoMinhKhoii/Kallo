...
## Data landing and embeddings

The 258 reviewed rows land through
`supabase/migrations/20260812100213_nin_foods_enrichment.sql`. Regenerate the
migration deterministically from the pinned snapshot with:

```bash
bun scripts/enrich/nin-ingest.ts \
  --input scripts/enrich/input/nin-foods-853.json \
  --db-names scripts/enrich/input/db-names.tsv \
  --emit-migration
```

The older `--apply` stage remains available for local diagnostics, but it is
superseded by the migration and must not be used to land NIN data in shared or
production databases.

For a dev database that already contains stale `nin_web_%` rows, `--reapply`
deletes and reinserts the constructed NIN rows in one transaction. The delete
also removes their stored vectors, so include `--embed` even though `state` is
not part of the embedding input:

```bash
bun --env-file=.env.local scripts/enrich/nin-ingest.ts \
  --input scripts/enrich/input/nin-foods-853.json \
  --db-names scripts/enrich/input/db-names.tsv \
  --reapply --embed
```

Embeddings are deliberately excluded from the SQL migration because they are
derived through an external API and cannot be regenerated byte-for-byte. After
the migration is applied, run the post-migration backfill. When no local
`inserted-ids.json` artifact exists, `--embed` deterministically uses all 258
constructed migration IDs:
