...
| `apple-sign-in/` | the sealed Sign in with Apple refresh token kept only so deletion can revoke it: link (exchange + subject check + seal + upsert), read for the deletion outbox, open + revoke; `contracts.ts` holds the route body and the Apple-subject lookup |
