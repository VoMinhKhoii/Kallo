...
| `20260925120000_circle_blocks_and_reports.sql` | A (Drizzle) | `friendships.blocked_by` (who placed a block — only they may unblock); `content_reports` table (target kind / reason / status CHECKs, one row per reporter per target, status + created_at triage index) |
| `20260925120100_content_reports_rls.sql` | B (Manual) | RLS on `content_reports`: a reporter may insert and read back their own rows; no UPDATE/DELETE policy (triage is admin-only, server-side) |
