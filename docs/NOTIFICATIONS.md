...
- **Aggregation**: upsert on `(recipientId, groupKey)` partial unique index while the row is open — "X and 2 others reacted". `actorIds` holds the aggregate's **full** deduplicated membership, newest first, and `actorCount` is derived from it (`cardinality`), so the total is exact and any actor can be retracted. Audiences are bounded (≤10 friends, ≤50 group members), so the array stays small; the UI renders the first two faces. Accepted operational bound: the audience cap holds at any *instant*, so a long-lived open aggregate can exceed it through churn (members who joined, acted, and left). **Retention does not bound this**, and the design does not claim it does: every refresh resets `created_at`, so the 90-day cron caps an open aggregate's *inactivity*, not its lifetime — an aggregate that keeps receiving activity lives, and keeps growing its membership array, for as long as that activity continues, until the recipient reads it and closes it. The bound that actually holds is the product cap on who can act at all: the array can only ever accumulate distinct actors with access to that one object, so its worst case is the set of people who have ever been in the ≤50-member group (or ≤10-friend circle) behind it, and only on a row the recipient never opened. At those sizes no hard array guard is warranted; if the caps ever move by an order of magnitude, a slice on the SET clause is the place to add one.
...
- **At most one push per open aggregate *per visit cycle*** — `notify()` returns the recipients whose row it INSERTED **plus** those whose event re-badged a row that had already been SEEN; a refresh of a row that is still unseen returns nobody. So the device knocks when an aggregate opens, stays quiet for the second through eighth reaction you have not looked at yet, and knocks again on the first event after you have looked. Push and badge therefore fire on exactly the same edges — **one push per badge edge, under concurrency too**: the classifying pre-select takes the open row `FOR UPDATE`, so two simultaneous events on the same seen row serialise (the second reads the first's committed `seen_at IS NULL` and calls itself a silent refresh), and two simultaneous events on a *missing* row are arbitrated by the open-aggregate unique index (the loser lands on the UPDATE branch with no pre-select entry, so it is neither an insert nor a re-badge and pushes nothing). The in-app row is the durable record; the push is only the knock.
...
flowchart TD
    ...
    G4 -- "yes, and it is still UNSEEN\n(not looked at yet)" --> U[UPDATE row: add actor,\nrecount, re-surface\nSILENT refresh]
    G4 -- "yes, but it was already SEEN\n(looked at since it last badged)" --> RB[UPDATE row: add actor,\nrecount, reset seenAt\nRE-BADGE — new visit cycle]
...
**Gate 5 fires on the same edges the badge does.** `notify()` reports back the recipients whose row Postgres actually inserted (`RETURNING (xmax = 0)`) **plus** those whose event landed on an already-SEEN open row, and the producers hand exactly that list to `after(() => sendNotificationPush(...))`. An event that aggregated into a row the recipient has not looked at yet updates the feed silently — otherwise the eighth reaction on a meal you have not opened would be the eighth push — but once the visit has ended that cycle, the next event knocks again rather than re-badging into a device that stays quiet. (To tell the two apart, `notify()` reads the open rows' `seen_at` **`FOR UPDATE`** in the same transaction immediately before the upsert overwrites it — one small indexed lookup over at most one row per recipient in the round. The lock is what makes the classification correct under concurrency: it holds each matched row until this transaction commits, so a second `notify()` on the same seen row blocks on the select rather than reading the pre-reset `seen_at`, and when it proceeds it sees `seen_at IS NULL` and classifies itself as a silent refresh — one push per badge edge instead of two. The recipient's own `markSeen` serialises through the same lock. The insert race needs no lock and gets none: when no open row exists there is nothing to lock, both transactions read an empty map and both attempt the INSERT, and the open-aggregate unique index arbitrates — the loser blocks on the index entry, takes the `ON CONFLICT` UPDATE branch, and reports neither `xmax = 0` nor a pre-select entry, so it is not push-worthy. Lock ordering is not a deadlock hazard because every caller locks through this one query shape.) `chat.message` never enters the row branch at all: it is push-only by design, writes nothing, and joins the flow at the FCM eligibility check.
...
Tests: `lib/domain/notifications/__tests__/` (input shaping, conflict-key spreading, the three push-set outcomes — insert, seen re-badge, silent refresh — self-skip, retract paths) + per-producer recipient assertions in existing action test folders.
