...
- **Acting on the offer is what reads the notification row.** The invite card is not a link — nothing else on it can fire `markRead` — so Accept and Dismiss both mark the row read on success (fire-and-forget, feed invalidation chained to its settle). Without that, `read_at` would stay null forever: the aggregate never closes, and a later re-share would land on this same open row and rewrite it instead of starting a fresh row beside it, contradicting the "read rows are immutable history" invariant of the lifecycle FSM. Resolved cards render a static chip and stay unread by design (there is nothing to act on).
...
**Push audiences that have no row are captured at write time.** `chat.message` is the one event whose recipients are not computed by `notify()` inside the producing transaction (Gate 3 — it writes no row). They must therefore be selected in the producer's own scope, alongside the message insert, and handed to `sendChatMessagePush({..., recipientIds})`. Querying `chat_group_members` inside the `after()` callback instead would read the membership as it stands *after* the commit, so somebody who joined in the interim would receive a lock-screen preview of a message sent before they were in the room. `push.ts` takes the list as given and only loads tokens and locales for it.
