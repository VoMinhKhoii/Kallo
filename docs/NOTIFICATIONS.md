...
- `activity-sections.tsx` — New / Last 30 days / Older buckets, presentation-only; "New" = `seenAt` null **in the fetched snapshot** (so rows don't jump sections mid-visit); infinite scroll via the `thread-feed.tsx` IntersectionObserver idiom (don't reuse ThreadFeed itself — wrong strings/separators).
...
- `desktop-sidebar.tsx`: `showBadge` for activity when `useUnseenNotificationCount() > 0` (follows the existing groups/inviteCount line).
- `mobile-nav-list.tsx`: badge on the activity drawer row (same plumbing as `inviteCount`).
