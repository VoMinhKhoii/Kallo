...
- ~~**Reaction and reply notifications deep-link to `/circle`, not to the meal.**~~ RESOLVED: the Circle thread route (`/circle/<shareId>`) exists, and `share.reaction` / `share.reply` / `share.logged` now open it from `objectId` on both surfaces — `notificationHref()` on web and `pushDestinationFor()` in `apps/mobile-flutter/lib/features/notifications/logic/push_tap_routing.dart` for an iOS push tap. A payload without a usable `objectType: 'share'` + `objectId` still falls back to `/circle` (`friend.joined` → `/circle` is already the right destination; `group.added` deep-links via `targetId`).
