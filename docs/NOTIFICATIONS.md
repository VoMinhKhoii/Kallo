...
| `share.invite_accepted` | sender | `acceptMealShareInviteAction` — `invite-response.ts`; `stageCheatInviteAction` — `stage-cheat-copy.ts` (fired when the cheat offer is TAKEN, not when its sliders are confirmed) | dismiss stays silent (LinkedIn norm: no rejection signal) |
...
**Sandbox vs production**: the token a build receives is only valid on the matching host. Development-signed Xcode builds (Debug/Profile) mint **sandbox** tokens (server needs `APNS_PRODUCTION=false`); Ad Hoc, TestFlight and App Store builds use distribution signing and mint **production** tokens (`APNS_PRODUCTION=true`). A mismatch answers `400 BadDeviceToken` on every send — which the server deliberately does **not** treat as a dead token, so a misconfigured environment loses pushes but never deletes registrations.
...
**Rows already written are durable point-in-time snapshots, with one exception: a block.** This is the Instagram model, and it is not a gap in the rule above: unfriending someone does not un-send the message they already received, it stops the next one. A later unfriend ends the person's participation in *future* fan-out; the activity they were legitimately told about at the time stays in their feed until retention removes it.

**A block purges.** `blockFriend` (`lib/actions/moderation/blocks.ts`) deletes, in the block's transaction, every row either person holds whose `actor_ids` contains the other, grouped rows included, since a grouped row's `data` preview may be the blocked person's words. Unblocking does not restore them. `notify()` then refuses new rows between the two (`dropBlocked`). Doing this at the write keeps the activity list, its cursor pagination and the badge count on plain recipient-scoped reads with no block rule of their own. A read-time filter would run after the page `LIMIT`, which can empty a page, and would leave the badge counting rows the list hides. Known gap: a `notify()` whose block check ran just before the block committed can still insert one row after the purge. Nothing hides that row, but every later event between the two is dropped.
