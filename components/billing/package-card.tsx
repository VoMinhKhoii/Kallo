...
export function PackageCard({
  ...
  const type = rcPackage.packageType;

  const isAnnual = type === PackageType.Annual;
  const isLifetime = type === PackageType.Lifetime;
  const featured = isAnnual;
    ...
    >
      ...
      <span className="mt-3 inline-flex items-center gap-1.5 font-medium text-[13px] text-nham-text">
        {pending ? (
          ...
          <Check className="h-3.5 w-3.5 text-nham-accent" />
        )}
