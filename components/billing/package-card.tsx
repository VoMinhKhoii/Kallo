...
export function PackageCard({
  ...
  return (
    <button
      ...
      aria-busy={pending}
      className={cn(
        'relative flex flex-col rounded-2xl border bg-white px-4 py-4 text-left transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-kallo-accent disabled:opacity-60',
        isAnnual
          ? 'border-kallo-accent/70 hover:border-kallo-accent'
          : 'border-kallo-border/70 hover:border-kallo-border'
      )}
    >
      {isAnnual && (
        <span className="absolute -top-2 right-3 rounded-full bg-kallo-accent px-2 py-0.5 font-medium text-[10px] text-white uppercase tracking-[0.1em]">
          {t('annualHint')}
        </span>
      )}

      <span className="text-[13px] text-kallo-text-muted">{planName}</span>

      <span className="mt-1 flex items-baseline gap-1">
        <span className="font-serif text-2xl text-kallo-text">
          {product.price.formattedPrice}
        </span>
        <span className="text-[13px] text-kallo-text-muted">{cadence}</span>
      </span>

      <span className="mt-3 inline-flex items-center gap-1.5 font-medium text-[13px] text-kallo-text">
        {pending ? (
          <Loader2 aria-hidden="true" className="h-3.5 w-3.5 animate-spin" />
        ) : (
          <Check aria-hidden="true" className="h-3.5 w-3.5 text-kallo-accent" />
        )}
