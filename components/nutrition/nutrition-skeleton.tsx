  ...
  return (
    <main className="flex-1 overflow-y-auto px-4 py-4 pb-20 sm:px-6 lg:px-8">
      <div
        ...
        className="mx-auto flex max-w-2xl flex-col gap-5 motion-safe:animate-pulse"
      >
        {/* Header — eyebrow + range toggle */}
        <div className="flex items-center justify-between">
          <div className="h-3 w-28 rounded-full bg-nham-track" />
          <div className="h-8 w-36 rounded-full bg-nham-track" />
        </div>
        ...
        <div className="rounded-[1.375rem] bg-card p-5 shadow-[0_10px_32px_rgba(44,36,22,0.05)]">
          <div className="flex items-start justify-between">
            <div className="h-10 w-40 rounded-full bg-nham-track" />
            <div className="h-3 w-20 rounded-full bg-nham-track" />
          </div>
          <div className="mt-4 h-2 w-full rounded-full bg-nham-track" />
          <div className="mt-3 flex justify-center gap-5">
            {Array.from({ length: 3 }, (_, index) => (
              <div
                key={index}
                className="h-3 w-16 rounded-full bg-nham-track"
              />
            ))}
          </div>
        </div>
          ...
          <div key={group} className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {Array.from({ length: 6 }, (_, index) => (
                <div
                  key={index}
                  className="space-y-2 rounded-2xl border border-nham-border/60 bg-card p-3"
                >
                  <div className="flex items-center justify-between">
                    <div className="h-3 w-16 rounded-full bg-nham-track" />
                    <div className="h-3 w-8 rounded-full bg-nham-track" />
                  </div>
