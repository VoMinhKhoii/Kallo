...
export function TimelineSidebar({
  ...
  return (
    <nav
      className="flex h-full w-[212px] shrink-0 flex-col gap-4 overflow-y-auto border-border/40 border-r py-4 pr-3"
        ...
        return (
          <div key={month.key} className="flex flex-col">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 transition-colors hover:text-nham-text"
            >
              <span
            ...
            {/* Separator always visible under month header */}
            <div className="mb-1 h-px rounded-sm bg-neutral-100" />

            {isMonthExpanded && (
              <div id={`month-${month.key}`} className="flex flex-col">
                  ...
                  return (
                    <div key={week.key}>
                      ...
                      <button
                        ...
                        className={cn(
                          'flex w-full items-center gap-3 rounded-xl px-3 py-4 transition-colors',
                          hasSelectedDay && 'bg-nham-accent/30',
                          !hasSelectedDay && 'hover:bg-nham-hover/40'
                        )}
                      >
                        <span
                          className="flex-1 text-left font-semibold text-base text-foreground"
                          style={{ fontFamily: 'DM Sans, sans-serif' }}
                        >
                          {week.label}
                        </span>
                      ...
                      {/* Days tree */}
                      {isWeekExpanded && (
                        <div id={`week-${week.key}`} className="flex pl-4">
                          ...
                          <ul className="-ml-0.5 flex flex-col">
                            {week.days.map((date) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="h-3 w-[14px] shrink-0 rounded-bl-lg border-nham-accent border-b-2 border-l-2" />
                                  <button
                                    type="button"
                                    onClick={() => onSelectDate(date)}
                                    aria-current={isActive ? 'date' : undefined}
                                    className={cn(
                                      'flex flex-1 items-center rounded-xl px-3 py-3',
