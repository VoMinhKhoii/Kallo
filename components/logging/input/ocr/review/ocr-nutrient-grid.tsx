 ...
 * charted `--kallo-chart-*` trio, hue-split — not the quiet `--kallo-macro-*`
...
const MACRO_IDENTITY: Record<string, { Icon: LucideIcon; className: string }> =
  {
    calories: { Icon: Flame, className: 'text-kallo-accent' },
    proteinGrams: { Icon: Beef, className: 'text-kallo-chart-protein' },
    carbsGrams: { Icon: Wheat, className: 'text-kallo-chart-carbs' },
    fatGrams: { Icon: Droplet, className: 'text-kallo-chart-fat' },
  };
...
function NutrientField({
  ...
  return (
    <div className="min-w-0">
      <label
        htmlFor={item.id}
        className="flex items-center gap-1.5 text-[12px] text-kallo-text-muted"
      >
        {identity && (
          <identity.Icon
        ...
        {item.required && <span className="text-kallo-danger">*</span>}
      </label>
      <div className="mt-1 flex items-baseline gap-1.5">
        <input
          ...
          className={`w-full min-w-0 border-none bg-transparent p-0 font-medium tabular-nums placeholder:text-kallo-text-muted/40 focus:outline-none ${inputSize} ${
            item.hasError ? 'text-kallo-danger' : 'text-kallo-ink'
          }`}
        />
        <span className="shrink-0 text-[12px] text-kallo-text-muted">
          {item.unit}
        </span>
      </div>
      ...
      <div
        className={`mt-1 transition-colors ${
          item.hasError ? 'h-[1.5px] bg-kallo-danger' : 'h-px bg-kallo-border'
        }`}
      />
      {item.hasError && (
        <p id={errorId} role="alert" className="mt-1 text-kallo-danger text-xs">
          {item.errorText}
        </p>
      )}
    </div>
