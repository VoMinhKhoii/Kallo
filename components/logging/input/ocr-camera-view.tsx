...
import type {
  ...
  OcrCameraError,
  OcrCameraResolution,
} from '@/hooks/meals/ocr-camera-types';
import { OcrCameraControls } from './ocr-camera-controls';
...
export function OcrCameraView(props: OcrCameraViewProps) {
  if (props.cameraError) {
    return (
      <div
        role="alert"
        className="flex aspect-[4/3] w-full flex-col items-center justify-center gap-2 rounded-2xl border border-nham-border bg-black p-6 text-center text-white sm:aspect-video"
      >
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl border border-nham-border bg-black shadow-sm sm:aspect-video">
        <video
          ref={props.videoRef}
          autoPlay
          playsInline
        ...
        />
        <div className="pointer-events-none absolute inset-0 m-3 rounded-xl border-2 border-nham-accent/70 border-dashed sm:m-6" />
