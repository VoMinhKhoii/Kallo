    ...
    <div className="flex h-full min-h-[180px] flex-col items-center justify-center gap-3 rounded-[1.5rem] border border-nham-border/60 bg-card p-4 text-center text-nham-stone text-sm shadow-[0_10px_32px_rgba(44,36,22,0.05)] dark:shadow-[0_10px_32px_rgba(0,0,0,0.15)]">
      ...
      {actionLabel && onAction ? (
        <button
          type="button"
          onClick={onAction}
          className="rounded-full bg-nham-btn px-4 py-2 font-semibold text-white text-xs transition-colors hover:bg-nham-btn-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nham-accent/60"
        >
          {actionLabel}
        </button>
      ) : null}
