  ...
  return (
    ...
    >
      ...
      <nav className="hidden h-full w-[252px] shrink-0 flex-col gap-3 overflow-hidden border-border/40 border-r py-3 pr-3 md:flex">
        <div className="h-3 w-20 rounded-full bg-nham-track motion-safe:animate-pulse" />
        <div className="flex flex-col gap-2">
          {Array.from({ length: 7 }, (_, index) => (
            <div
              key={index}
              className="h-8 rounded-lg bg-nham-track motion-safe:animate-pulse"
      ...
      <main className="flex min-w-0 flex-1 flex-col self-stretch overflow-hidden">
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-3 py-3 sm:px-6 sm:py-4">
          <p className="mx-auto mb-6 w-full max-w-3xl pl-6 font-sans-display text-nham-text-muted text-sm sm:pl-12">
            Pulling up your day…
          </p>
        ...
        <div className="px-3 pb-3 sm:px-6 sm:pb-4">
          <div className="mx-auto h-14 w-full max-w-3xl rounded-2xl border border-nham-border/60 bg-nham-hover/20 motion-safe:animate-pulse" />
        </div>
      </main>
    </div>
  );
