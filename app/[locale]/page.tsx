...
import { SignedInRedirect } from '@/components/landing-page/signed-in-redirect';
...
/**
 * The marketing root, prerendered as a static shell per locale (Cache
 * Components). Everything that depends on the request — the `?auth=` /
 * `?next=` invite intent, the runtime Google client ID, and the signed-in
 * redirect — lives in the two leaves inside `<Suspense>` at the bottom, so
 * none of it holds the page back.
 */
export default async function Home({
  ...
  return (
    <AuthProvider initialTab="sign-up">
      ...
      <Suspense fallback={null}>
        <OAuthErrorToast />
        <WaitlistStatusToast />
        <AuthRequestConfig searchParams={searchParams} />
