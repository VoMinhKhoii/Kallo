...
**Nhẩm** (read: "n-yum", roughly — to *recite quietly under one's breath*, also colloquially "to estimate roughly in your head") is an AI-powered Vietnamese meal tracker. You describe your meal the way you'd describe it to a friend — `2 mực kho mặn + 50gr nạc dăm luộc + 1 chén cơm + canh chua` — and Nhẩm decomposes it into Vietnamese ingredients, looks up real Vietnamese food-composition data, layers in your personal cooking-habit profile (oil usage, rice portion, broth tendencies), and returns a *bounded* calorie + macro estimate. No barcode scanning. No 12-step food-search. No Western databases pretending phở bò and phở gà are the same dish.
...
- **GitHub** — `VoMinhKhoii/Nham` — https://github.com/VoMinhKhoii/Nham
  - `app/globals.css` — full color token table (the `--nham-*` namespace)
...
**Voice.** Calm, plain, slightly poetic. Nhẩm sounds like a thoughtful older sibling who cooks. It explains, it doesn't sell. It admits uncertainty rather than faking precision.
...
- Color is always inherited from `currentColor`. Inside accent-tinted contexts (e.g. the sparkle in the dark pill), set color explicitly to `var(--nham-accent)`.
...
├── assets/
│   ├── logo-wordmark.svg       ← "Nhẩm" wordmark in Lora
...
### 1. Generic shadcn `destructive` instead of `--nham-danger`

❌ `bg-destructive/10 text-destructive` (renders cold red `oklch(0.577 0.245 27.325)`)
✅ `bg-nham-danger/10 text-nham-danger` (terracotta `#d37b69`)
...
The `h-1.5 w-1.5 rounded-full bg-nham-accent` "leading dot" pattern is fine **once per section, as a status indicator**. It is NOT fine as decoration in front of every row label. Nutrient rows, the verdict hero, the composition legend, and the macro pattern all use it simultaneously — that's four dots before three pieces of text. Result: dot fatigue.

**Rule**: a tiny dot earns its place only when it encodes status. If every dot is the same color (`bg-nham-accent`) and conveys no information, delete it.

### 3. Trend-arrow icons

`TrendingUp` / `TrendingDown` / `ChevronRight` are SaaS-dashboard tropes. The Nhẩm voice is conversational, not analytical. Weight progress should be written, not arrowed:
...
The `<span class="bg-destructive/10 text-destructive"><TrendingDown />Behind</span>` pattern in `progress-story.tsx` is the worst offender — it lights up red on a warm cream background, screams "you failed", and uses an icon to do it. The Nhẩm voice for the same situation is the dashboard's own copy: *"The trend is softer than the plan right now."* — Lora italic, no pill, no icon, no color.

### 5. Card opacity gymnastics

`bg-card/55`, `bg-card/80`, `bg-nham-surface/70` are AI-generated, not designed. Real cards are solid `bg-white` (`#fff`) or solid `bg-nham-surface` (`#fefbf6`). Hierarchy comes from **borders**, not alpha layering.
...
3. **Pick the right scale of reuse.**
   - **Token-level** (always reuse): `--nham-*` colors, the type families, radii, shadows, the icon allowance, the eight Hard Rules.
...
- **Card 25** is the venture — a full page mocked up as a *session canvas* on the stone (`--nham-track`) background instead of cream, with a thin week-strip at the top and exercise blocks anchored by a small line-art sketch. Different mental model from meal logging, different "room" within the same house. Sets are rendered in a clean four-column grid (**Set / Weight / Reps / RPE**) with the unit pinned to the right of each cell — `kg`, `reps`, `/10`.
...
The same recipe extends to any new feature. If you're not sure where to start, mock up one screen in the *wrong* way (the way a generic fitness/sleep/mood app would do it), then translate it surface-by-surface back into Nhẩm voice using the rules above.
