...
1. **`destructive` token** instead of `--nham-danger` — `destructive` renders cold red, breaks the warm palette on sight. Always `bg-nham-danger/10 text-nham-danger`.
2. **Tiny-dot disease** — `h-1.5 w-1.5 rounded-full bg-nham-accent` as decoration before every label. Dots earn their place only when they encode status.
...
5. **`bg-card/55` opacity tricks** — use solid `bg-white` or `bg-nham-surface`; hierarchy comes from borders, not alpha.
...
- **Lora is never bold** — 400 normal or 300 italic light. The signature treatment is the second clause of a headline in italic 300 at `--nham-accent` (`#c9a87c`).
