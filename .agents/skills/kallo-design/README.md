...
| **Surface states** | `components/shared/surface-state/surface-state.tsx` | Every empty, error, 404 and offline surface composes this: Koboyo illustration in ink → Lora 24 title → 14px muted line → at most one umber CTA. Errors get `role="alert"`; `compact` is the in-card size. No bare one-line empties, no icon-on-a-disc. |
