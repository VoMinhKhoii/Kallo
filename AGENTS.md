...
- **Pre-Coding Skill Check**: BEFORE ANY coding task — even the slightest edit — invoke the relevant React skill(s) from Vercel and review matching rules. Only begin coding after relevant rules have been reviewed. This prevents anti-patterns before they're written.
  ...
  **Which skill to invoke:**
  ...
  | Skill | When to use |
  |-------|-------------|
  | `feature-workflow` | **Invoke FIRST for any non-trivial fix, feature, or refactor — before exploring or editing.** The umbrella dev loop distilled from this repo's session history: orient/probe state → decision gate on design forks → implement with per-phase verification → `verify-before-done` → `grill-your-own-work` → ship. The router itself is a composition (not separately validated); the discipline skills it routes to were individually blind-validated — 12 clean wins + 1 void, 2 losses — with records in `.claude/skills/_evidence/`. Note: an eighth skill, `absorb-steering`, FAILED validation and was removed from the registry; its failure record lives in `.claude/skills/_evidence/failed-skills/`. |
