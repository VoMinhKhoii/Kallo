 ...
 *
 * Usage:
 *   bun --env-file=.env.local scripts/data/usda/import_to_supabase.ts --csv data/usda_sr/usda_sr_legacy.csv
 *   bun --env-file=.env.local scripts/data/usda/import_to_supabase.ts --csv data/usda_sr/usda_sr_legacy.csv --dry-run
 *
...
if (csvIdx === -1 || !args[csvIdx + 1]) {
  console.error(
    'Usage: bun --env-file=.env.local scripts/data/usda/import_to_supabase.ts --csv <path>'
  );
  process.exit(1);
}
...
if (!dryRun && !process.env.DATABASE_URL) {
  console.error(
    'Missing DATABASE_URL. Run with:\n  bun --env-file=.env.local scripts/data/usda/import_to_supabase.ts --csv <path>'
  );
  process.exit(1);
}
