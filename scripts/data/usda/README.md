...
python3 scripts/data/usda/download_usda_sr.py \
...
bun --env-file=.env.local scripts/data/usda/import_to_supabase.ts \
...
```bash
bun --env-file=.env.local scripts/db/backfill_embeddings.ts
```
...
```bash
python3 scripts/data/usda/test_download.py
```
