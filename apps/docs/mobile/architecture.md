...
| Auth provider | `google_sign_in` ^6.3 |
...
- **Data:** `api_client.dart` hits `/api/v1`. Note Drizzle decimals serialize as **strings**, and
  ...
  the same defaults as the RN/web app.
...
`apps/mobile` (React Native / Expo) is an earlier port of the same product. Both clients are
intentionally feature-equivalent and share the backend + Supabase project. The Flutter app's
bundle id was aligned to the RN app's (`com.khoivo.nham`) so they occupy one TestFlight record —
