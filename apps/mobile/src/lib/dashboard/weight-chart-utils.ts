/**
 * Vendored VERBATIM from components/dashboard/progress/weight-chart-utils.ts —
 * keep in sync.
 *
 * The web module is a runtime VALUE import; copied here so Metro can bundle it.
 * `TimeRange` is a type-only import (erased by Babel) and resolves via `@/lib`.
 * `Intl.DateTimeFormat` and `new Date()` are both fine in app code.
 *
 * Provides X-axis ticks + a label formatter: W1/W2/…/Now for 30d,
