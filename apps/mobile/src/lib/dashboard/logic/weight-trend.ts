/**
 * Vendored VERBATIM from lib/dashboard/weight-trend.ts — keep in sync.
 *
 * The web module is a runtime VALUE import from `@/lib`, which Metro will not
 * resolve, so the logic is copied here. Only the type imports differ (they are
 * type-only and erased by Babel, so they resolve fine via the `@/lib` alias).
 *
 * Produces the weight-chart callout (status / delta / projection).
 */
