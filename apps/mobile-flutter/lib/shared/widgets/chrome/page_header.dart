...
/// A pushed page's ONE top bar: a 44×44 back chevron with the page title on
/// the same line, 28/600 (`kPageTitle`) in ink.
///
/// Every pushed sub-page renders the identical bar — same back affordance,
/// same slot, same inset — so navigating never changes its shape. The only
/// thing that morphs is [title]: "Settings" at the root of that stack, the
/// section's own name inside it, "Send feedback" on the feedback form.
/// Because the title lives here, no sub-page repeats it in the body.
///
/// It grew up as the settings stack's header; it moved to `shared/` when
/// feedback — pushed from the settings list but living in its own feature —
/// became the second consumer.
///
/// The title is LEFT-aligned beside the chevron rather than centred between
/// two 44pt slots (native pass, 2026-08-31): at 28/600 it is the page's
...
class PageHeader extends StatelessWidget {
  ...
  /// Defaults to `maybePop` on the enclosing navigator — inside settings that
  /// is the nested one, so a drill-in's back goes up one level instead of
  /// closing the screen.
