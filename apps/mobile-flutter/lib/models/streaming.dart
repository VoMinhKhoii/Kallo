...
class StreamErrorEvent extends StreamEvent {
  ...
  /// HTTP status when the error came from a non-2xx response on the SSE
  /// endpoint (vs. an in-stream `error` frame, where it's null). Lets consumers
  /// route a `402 Payment Required` (feature locked, post Phase E) to the
  /// paywall. `null` for in-stream error frames.
  final int? status;
  ...
  /// True when the server signalled the feature is locked (HTTP 402) — the
  /// client should route to the paywall rather than showing a retry error.
