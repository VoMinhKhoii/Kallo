...
class SavePlanScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // No SafeArea around this: the sweep has to start at the very top of the
      // screen, behind the status bar, and the auth face has to reach the very
      // bottom — its slice is aligned to that same full-screen box. Only the
      // chrome is inset, and [AuthPage] insets its own buttons at the bottom.
      body: LayoutBuilder(
        builder:
            (context, box) => Stack(
              children: [
                const Positioned.fill(child: StepBackdrop()),
                SafeArea(bottom: false, child: _content(context, box.biggest)),
              ],
            ),
      ),
    );
