...
void main() {
  test(
    'signed in, a screen with no payload still advances the SERVER step',
    () async {
      ...
      expect(
        api.posts.every((p) => p.data.isEmpty),
        isTrue,
  ...
  test(
    'the sink flips from draft to server the moment the session appears',
    () async {
      ...
      expect(
        container.read(onboardingSinkProvider),
        isA<DraftOnboardingSink>(),
      );
      await container
          .read(onboardingSinkProvider)
          .record(
            screen: 2,
