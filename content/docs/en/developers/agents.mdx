...
- **Reading a nutrition label from a photo.** `POST /api/v1/nutrition-label/scan`
  returns what it read and logs no meal. The photo and the scan outcome (what
  was read, or why it failed) are kept in private storage linked to the
  account, visible only to its owner and the Kallo team, until the account is
  deleted.
